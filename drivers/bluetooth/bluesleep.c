...
static void hsuart_power(int on)
{
	...
	if (on) {
		...
		if (clk_cnt >= 1) {
			BT_DBG("%s called. But HS Uart clock count is %d", __func__, clk_cnt);
		} else {
...
static void bluesleep_abnormal_stop(void)
{
	...
	if (!test_bit(BT_PROTO, &flags)) {
		BT_ERR("(%s) proto is not set. Failed to stop bluesleep", __func__);
		bsi->uport = NULL;
		return;
	}
...
void bluesleep_make_node(void)
{
	...
	if (proc_create("hostwake", 0660, sleep_dir, &proc_fops_hostwake) == NULL) {
...
static int bluesleep_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		BT_ERR("bluesleep_probe set input for host_wake is failed");
		goto free_bsi;
	}
	...
	if (ret) {
		BT_ERR("bluesleep_probe gpio_request for bt_wake is failed");
		goto free_bsi;
	} else {
		bsi->has_ext_wake = 1;
	}
	...
	if (ret) {
		BT_ERR("bluesleep_probe set input for bt_wake is failed");
		goto free_bsi;
	}
