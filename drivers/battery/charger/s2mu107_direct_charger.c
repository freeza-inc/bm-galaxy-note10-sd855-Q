...
//	union power_supply_propval value;
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		s2mu107_dc_state_manager(charger, DC_TRANS_SET_CURRENT_MAX);
		break;
...
static void s2mu107_dc_cal_target_value(struct s2mu107_dc_data *charger)
{
	...
	temp = vBatt * 2 * (1031);
	charger->ppsVol = (((temp / 1000) / DC_TA_VOL_STEP_MV) * DC_TA_VOL_STEP_MV) + 260 + (charger->vchgin_okb_retry * DC_TA_VOL_STEP_MV);
...
{
	int vchgin;
	int comp_pps_vol, comp_gap, diff;

	vchgin = s2mu107_dc_get_pmeter(charger, PMETER_ID_VCHGIN);
	if (vchgin < 0) {
		pr_info("%s Can't get Pomwer meter information!\n", __func__);
		return ;
	}

	/* In case of over vchgin */
	if (vchgin > charger->ppsVol + 50) {
		diff = (vchgin - charger->ppsVol);
		charger->ppsVol =
			((charger->ppsVol - diff) / DC_TA_VOL_STEP_MV) * DC_TA_VOL_STEP_MV;
		sec_pd_select_pps(charger->pd_data->pdo_pos, charger->ppsVol, charger->adjustIIN);
		s2mu107_dc_delay(charger, 200);
	...
	while (charger->ppsVol - 50 > vchgin) {
		s2mu107_dc_get_fg_value(charger, POWER_SUPPLY_PROP_CURRENT_NOW);
		comp_gap = ((comp_pps_vol - vchgin) / DC_TA_VOL_STEP_MV) * DC_TA_VOL_STEP_MV + DC_TA_VOL_STEP_MV;
		if (vchgin < 4100) {
		...
		comp_pps_vol += comp_gap;
		pr_info("%s comp_pps_vol : %d\n", __func__, comp_pps_vol);
		sec_pd_select_pps(charger->pd_data->pdo_pos, comp_pps_vol, charger->adjustIIN);
		s2mu107_dc_delay(charger, 300);
		vchgin = s2mu107_dc_get_pmeter(charger, PMETER_ID_VCHGIN);
...
static int s2mu107_dc_check_vbat_validity(struct s2mu107_dc_data *charger)
{
	...
	if (vBatt >= 3450) {
		...
		charger->chk_vbat_margin = (3400 - vBatt) * 3;
		return 1;
	}
}
...
static void s2mu107_dc_set_target_curr(struct s2mu107_dc_data *charger)
{
	...
	charger->targetIIN = charger->step_iin_ma;
...
static void s2mu107_dc_refresh_auto_pps(struct s2mu107_dc_data *charger, unsigned int vol, unsigned int iin)
{
	...
	if (s2mu107_dc_delay(charger, 500)) return;
...
#ifdef CONFIG_SEC_FACTORY
			next_state = DC_STATE_ADJUSTED_CC;
#else
...
static void s2mu107_dc_state_preset(void *data)
{
	...
	charger->pd_data->taMaxVol	= taMaxVol - 500;
	...
	charger->adjustIIN
		= (charger->ta_pwr_type == TA_PWR_TYPE_45W) ? \
		DC_TA_START_PPS_CURR_45W : DC_TA_START_PPS_CURR_25W;
...
static void s2mu107_dc_state_start_cc(void *data)
{
	...
	while (charger->ppsVol < charger->pd_data->taMaxVol) {
		vbatt = s2mu107_dc_get_pmeter(charger, PMETER_ID_VBATT);
		/* Rampup Step Minimized Conditions */
		if ((iin >= charger->step_iin_ma - 100)
				|| (vbatt >= charger->step_vbatt_mv - 200))
		...
		iin = s2mu107_dc_get_pmeter(charger, PMETER_ID_ICHGIN) +
			s2mu107_dc_get_pmeter(charger, PMETER_ID_IWCIN);
		pr_info("%s iin : %d, wait_ret : %d\n", __func__, iin, wait_ret);
		if (iin >= charger->step_iin_ma + 200) {
			break;
		}
		/* Check Over VBAT */
	...
	while (try_cnt++ < 50) {
		if (charger->ta_pwr_type == TA_PWR_TYPE_45W) {
			...
			iin = s2mu107_dc_get_pmeter(charger, PMETER_ID_ICHGIN) +
				s2mu107_dc_get_pmeter(charger, PMETER_ID_IWCIN);
			pr_info("%s after rampup iin : %d\n", __func__, iin);

			if (iin < charger->step_iin_ma + 200) {
				charger->ppsVol += DC_TA_VOL_STEP_MV;
				wait_ret = s2mu107_dc_send_verify(charger);
			} else if (iin > charger->step_iin_ma + 400) {
				charger->ppsVol -= DC_TA_VOL_STEP_MV;
				wait_ret = s2mu107_dc_send_verify(charger);
			} else
				msleep(50);
...
static void s2mu107_dc_state_sleep_cc(void *data)
{
	...
	vol = (!charger->is_rampup_adjusted) ? \
		charger->pd_data->taMaxVol : charger->ppsVol;

	s2mu107_dc_set_target_curr(charger);
	s2mu107_dc_refresh_auto_pps(charger, vol, charger->step_iin_ma);
...
static void s2mu107_dc_timer_work(struct work_struct *work)
{
	...
	if (!charger->is_charging) {
		pr_info("%s, DC is not charging. skip work\n", __func__);
		return ;
	}
		...
		charger->ppsVol -= DC_TA_VOL_STEP_MV;
			...
			if (charger->ta_pwr_type == TA_PWR_TYPE_25W) {
				iin = s2mu107_dc_get_pmeter(charger, PMETER_ID_ICHGIN) +
					s2mu107_dc_get_pmeter(charger, PMETER_ID_IWCIN);
				pr_info("%s iin : %d\n", __func__, iin);
				if (iin >= charger->targetIIN - 50) {
					s2mu107_dc_state_trans_enqueue(charger, DC_TRANS_RAMPUP_OVER_CURRENT);
					goto SKIP_PPS_ISR_SEND_MSG;
				}
			}
...
static irqreturn_t s2mu107_dc_fail_isr(int irq, void *data)
{
	...
	if (irq == charger->irq_ramp_fail
		...
		|| irq == charger->irq_wcin_rcp
		|| irq == charger->irq_chgin_rcp
		|| irq == charger->irq_plug_out
		...
		|| irq == charger->irq_tsd) {
		pr_info(" dc was stopped\n");
		charger->ps_health = POWER_SUPPLY_HEALTH_DC_ERR;
		charger->is_charging = false;
	} else if (irq == charger->irq_vchgin_okb) {
		pr_info(" VBUS is not ready\n");
		s2mu107_dc_get_pmeter(charger, PMETER_ID_VCHGIN);
		skip_fail = true;
...
static int s2mu107_dc_irq_init(struct s2mu107_dc_data *charger)
{
	...
	/* DC_INT_1 */
	charger->irq_wcin_rcp = irq_base + S2MU107_DC_IRQ1_DC_WCIN_RCP;
	ret = request_threaded_irq(charger->irq_wcin_rcp, NULL,
			s2mu107_dc_fail_isr, 0, "dc-wcin-rcp", charger);
	if (ret < 0)
		goto err_irq;
...
#ifdef CONFIG_CHARGER_S2MU107_DIRECT_SUPPORT_WEAK_TA
		if (charger->step_cv_cnt == 1)
			charger->targetIIN -= DC_TA_CURR_STEP_MA * 7;
		else
			charger->targetIIN -= DC_TA_CURR_STEP_MA;
#else
		charger->targetIIN -= DC_TA_CURR_STEP_MA;
...
{
	int vavg = 0, iin = 0, fix_vol = 0;
 
	iin = s2mu107_dc_get_pmeter(charger, PMETER_ID_ICHGIN) +
				s2mu107_dc_get_pmeter(charger, PMETER_ID_IWCIN);

	if ((iin < charger->targetIIN) && (charger->ta_pwr_type == TA_PWR_TYPE_25W)) {
		if ((charger->targetIIN - iin) > 200) {
			fix_vol = charger->ppsVol + 500;
			charger->targetIIN = charger->step_iin_ma;
		} else {
			fix_vol = charger->ppsVol + DC_TA_VOL_STEP_MV;
		}
		s2mu107_dc_refresh_auto_pps(charger,
			fix_vol, charger->targetIIN);
		charger->alarm_interval = RUSH_ALARM_INTERVAL;
	} else
		charger->alarm_interval = DEFAULT_ALARM_INTERVAL;
	...
	if (vavg >= charger->step_vbatt_mv) {
		s2mu107_dc_state_manager(charger, DC_TRANS_FLOAT_VBATT);
	}
}
