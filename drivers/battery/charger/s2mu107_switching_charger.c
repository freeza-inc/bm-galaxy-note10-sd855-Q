...
static bool s2mu107_sc_init(struct s2mu107_sc_data *sw_charger)
{
	...
	if (!factory_mode)
#endif
		s2mu107_sc_set_input_current_limit(sw_charger, 3000);
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval) {
...
#endif
			s2mu107_update_reg(sw_charger->i2c, S2MU107_SC_TEST3, 0x10, 0x30);
			...
			/* manual reset time 3s for R5 */
			s2mu107_update_reg(sw_charger->i2c_common, 0xE5, 0x0A, 0x0F);
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		if (val->intval) {
			...
			s2mu107_update_reg(sw_charger->i2c, 0x3C, 0x40, 0xC0);
	...
	case POWER_SUPPLY_PROP_FUELGAUGE_RESET:
		s2mu107_read_reg(sw_charger->i2c_common, 0xE3, &data);
		data |= 0x03 << 6;
		s2mu107_write_reg(sw_charger->i2c_common, 0xE3, data);
		msleep(1000);
		data &= ~(0x03 << 6);
		s2mu107_write_reg(sw_charger->i2c_common, 0xE3, data);
		msleep(50);
...
static void s2mu107_sc_icr_work(struct work_struct *work)
{
	struct s2mu107_sc_data *sw_charger = container_of(work,
			struct s2mu107_sc_data,
			icr_work.work);
	...
	if (val == 0x0) {
		s2mu107_update_reg(sw_charger->i2c, S2MU107_SC_CTRL1,
				0x76 << INPUT_CURRENT_LIMIT_SHIFT, INPUT_CURRENT_LIMIT_MASK);
		pr_info("%s: set ICR 3A\n", __func__);
	}

	s2mu107_read_reg(sw_charger->i2c, S2MU107_SC_CTRL1, &val);
	pr_info("%s : SC_CTRL1 %02x\n", __func__, val);
}
...
static irqreturn_t s2mu107_sc_chgin_isr(int irq, void *data)
{
	...
	if (val == 0x1 || val == 0x2 || val == 0x3 || val == 0x5) {
		...
		pr_info("%s: queue ICR 3A work (delay 250msec)\n", __func__);
		queue_delayed_work(sw_charger->charger_wqueue, &sw_charger->icr_work,
...
static int s2mu107_switching_charger_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&sw_charger->icr_work, s2mu107_sc_icr_work);
