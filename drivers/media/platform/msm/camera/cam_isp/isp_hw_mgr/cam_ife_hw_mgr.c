	...
	enum cam_ife_pix_path_res_id        csid_path)
{
	...
	*cid_res = NULL;
	...
	CAM_INFO(CAM_ISP, "path %d", csid_path);
	...
	struct cam_isp_in_port_info   *in_port)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		cam_ife_hw_mgr_put_res(&ife_ctx->res_list_ife_csid, &csid_res);
		CAM_INFO(CAM_ISP,
				"ISP_ACQUIRE CSID Path reserved hw=%d",
				hw_intf->hw_idx);
	...
	uint32_t  *num_pix_port, uint32_t  *num_rdi_port)
{
	...
	CAM_INFO(CAM_ISP, "ISP_Acquiring ipp: %d ppp: %d rdi %d", ipp_count, ppp_count, rdi_count);
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		CAM_INFO(CAM_ISP, "in_res_type %x", in_port->res_type);
					...
					void *config_hw_args)
{
	...
	CAM_DBG(CAM_ISP, "Enter ctx id:%d num_hw_upd_entries %d request id: %llu",
	...
	bool *mem_found)
{
	...
	for (i = 0; i < packet->num_io_configs; i++) {
		for (j = 0; j < CAM_PACKET_MAX_PLANES; j++) {
			...
			if (pf_buf_info && GET_FD_FROM_HANDLE(io_cfg[i].mem_handle[j]) ==
			...
			CAM_DBG(CAM_ISP, "port: 0x%x f: %u format: %d dir %d",
				io_cfg[i].resource_type,
				io_cfg[i].fence,
				io_cfg[i].format,
				io_cfg[i].direction);
