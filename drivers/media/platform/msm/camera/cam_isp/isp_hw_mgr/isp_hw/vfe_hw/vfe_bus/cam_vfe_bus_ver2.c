...
#define CAM_VFE_PDAF_CFG              0xCB4
#define CAM_VFE_DUAL_PD_CFG           0xFE0
#define CAM_VFE_VIOLATION_STATUS      0x7C
	...
	void *evt_payload_priv)
{
	...
	struct cam_vfe_bus_ver2_common_data *common_data;
	uint32_t val = 0;
	uint32_t pdaf_cfg, dual_pd_cfg, vfe_vio_sts;
	uint32_t i;
	void *addr;
	...
	/* Dump required configurations to debug */
	pdaf_cfg    = cam_io_r_mb(common_data->mem_base + CAM_VFE_PDAF_CFG);
	vfe_vio_sts = cam_io_r_mb(common_data->mem_base +
		CAM_VFE_VIOLATION_STATUS);
	CAM_INFO(CAM_ISP, "pdaf_cfg %x vfe_vio_status %x",
			pdaf_cfg, vfe_vio_sts);

	for (i = 0; i < 19; i++) {
		addr = common_data->mem_base + CAM_VFE_DUAL_PD_CFG + (4*i);
		dual_pd_cfg = cam_io_r_mb(addr);
		CAM_INFO(CAM_ISP, "Reg_offset %x Dual PD[%d] %x",
			CAM_VFE_DUAL_PD_CFG + (4*i),
			i, dual_pd_cfg);
	}

	cam_vfe_bus_dump_dmi_reg(common_data->mem_base, 221, 0, 0x44);
	cam_vfe_bus_dump_dmi_reg(common_data->mem_base, 221, 0, 0x45);
	...
	struct cam_irq_th_payload *th_payload)
{
	...
	CAM_INFO(CAM_ISP, "mem base to read %x debug_status_0 %x",
		bus_priv->common_data.mem_base,
		bus_priv->common_data.common_reg->debug_status_0);
