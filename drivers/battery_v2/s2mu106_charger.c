...
static void s2mu106_set_otg_iout(struct s2mu106_charger_data *charger, int iout);
static void s2mu106_set_uno_iout(struct s2mu106_charger_data *charger, int iout);
...
static void regmode_vote(struct s2mu106_charger_data *charger, int voter, int en)
{
	static int vote_status = -1;
	u8 set_val, reg;
	mutex_lock(&charger->regmode_mutex);

	pr_info("%s: %s voter: 0x%x\n", __func__, en ? "set" : "unset", voter);
		...
		struct s2mu106_charger_data *charger, bool enable)
{
	...
	if (!enable) {
		regmode_vote(charger, REG_MODE_OTG, false);
		...
		psy_do_property("wireless", set,
			POWER_SUPPLY_PROP_CHARGE_OTG_CONTROL, value);
		/* 1. OCP 1.2A setting */
		s2mu106_set_otg_iout(charger, 1200);
		...
		mdelay(10);
		/* 6. OTG Enable */
		regmode_vote(charger, REG_MODE_OTG, true);
		mdelay(10);
	...
	struct s2mu106_charger_data *charger, int onoff)
{
	...
	if (onoff > 0) {
		...
		regmode_vote(charger, REG_MODE_CHG|REG_MODE_BUCK, true);
		...
		regmode_vote(charger, REG_MODE_CHG, false);
	}
}
...
static void s2mu106_set_buck(
	...
	if (enable) {
		...
		mdelay(50);
		regmode_vote(charger, REG_MODE_CHG|REG_MODE_BUCK, false);
		/* auto async mode */
...
static int s2mu106_get_input_current_limit(struct s2mu106_charger_data *charger)
{
	...
	s2mu106_read_reg(charger->i2c, reg, &data);
	if (data < 0)
		return data;
...
static void s2mu106_set_uno(struct s2mu106_charger_data *charger, int en)
{
	...
	if (charger->otg_on) {
		...
		if (en) {
			...
			if (value.intval) {
				regmode_vote(charger, REG_MODE_TX, false);
	...
	if (en) {
		...
		if (factory_mode) /* doesn`t support TX_CHGIN_BUCK_MODE when factory mode */
			...
			regmode_vote(charger, REG_MODE_TX, true);
...
static void s2mu106_set_uno_iout(struct s2mu106_charger_data *charger, int iout)
{
	...
	if (iout < 900)
		reg = S2MU106_SET_OTG_TX_OCP_500mA;
	else if (iout >= 900 && iout < 1200)
		reg = S2MU106_SET_OTG_TX_OCP_900mA;
	else if (iout >= 1200 && iout < 1500)
		reg = S2MU106_SET_OTG_TX_OCP_1200mA;
	else if (iout >= 1500)
		reg = S2MU106_SET_OTG_TX_OCP_1500mA;

	if (reg)
		s2mu106_update_reg(charger->i2c, S2MU106_CHG_CTRL3,
						reg << SET_TX_OCP_SHIFT, SET_TX_OCP_MASK);

	s2mu106_read_reg(charger->i2c, S2MU106_CHG_CTRL3, &reg);
	pr_info("@Tx_mode %s: CHG_CTRL3 (0x%x)\n", __func__, reg);
}
...
static void s2mu106_set_otg_iout(struct s2mu106_charger_data *charger, int iout)
{
	...
	if (reg)
		s2mu106_update_reg(charger->i2c, S2MU106_CHG_CTRL3,
						reg << SET_OTG_OCP_SHIFT, SET_OTG_OCP_MASK);
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (!charger->psy_fg)
			return -EINVAL;
		ret = power_supply_set_property(charger->psy_fg, POWER_SUPPLY_PROP_CHARGING_ENABLED, &value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval) {
...
#endif
			/* EN_MRST, MRSTBTMR3.0s (can be changed) */
			s2mu106_update_reg(charger->i2c, 0xE5, 0x0A, 0x0F);
			...
			/* RST_SW_CHG (CHG VIO Reset On) because of MRST 2.0s */
			s2mu106_update_reg(charger->i2c, 0xEF, 0x1, 0x1);

			/* Switchingfor fuel gauge to get Battery voltage */
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
...
#if 0 /* SYS Output 4.2V (Default; Skip below) */
			/* enable EN_JIG_AP */
...
#else
			pr_info("%s: factory voltage regulation (4200)\n", __func__);
#endif
			break;
		case POWER_SUPPLY_EXT_PROP_CURRENT_MEASURE:
			...
			if (val->intval) {
				...
				/* VBUSUVLO Disable(VBUS Input IR Drop) */
				s2mu106_update_reg(charger->i2c, 0x39, 0xC0, 0xC0);
...
static irqreturn_t s2mu106_bat_isr(int irq, void *data)
{
	...
	pr_info("%s - 1, 0x%02x\n", __func__, val);
	if (val & 0x02) {
		regmode_vote(charger, REG_MODE_OTG_TX, false);
