...
static ssize_t sec_ts_es_enabled_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
	struct sec_ts_data *ts = container_of(sec, struct sec_ts_data, sec);
...
static ssize_t sec_ts_es_enabled_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_SET_ES_ENABLE, &ts->es_enable, 1);
	if (ret < 0) {
		input_err(true, &ts->client->dev,
				"%s: failed to set es_enable\n", __func__);
		goto out;
	}

	if (ts->es_enable) {
		/* set data type */
		mode = SEC_TS_SET_VALUE_MUTU_DATA;
		ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_MUTU_RAW_TYPE, &mode, 1);
		if (ret < 0) {
			input_err(true, &ts->client->dev, "%s: Set mutual data type failed\n", __func__);
			goto out;
		}
		mode = SEC_TS_SET_VALUE_SELF_DATA;
		ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_SELF_RAW_TYPE, &mode, 1);
		if (ret < 0) {
			input_err(true, &ts->client->dev, "%s: Set self data type failed\n", __func__);
			goto out;
		}
		sec_ts_delay(30);
	} else {
		/* release data monitory */
		...
		memset(ear_data, 0x00, EAR_DATA_READ_LENGTH);
		memset(hover_data, 0x00, HOVER_DATA_READ_LENGTH);
	}

	input_report_key(ts->input_dev, KEY_QEEXO_ES_ENABLE, ts->es_enable);
...
static ssize_t sec_ts_es_touch_count_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
	struct sec_ts_data *ts = container_of(sec, struct sec_ts_data, sec);
...
#ifdef QEEXO_EAR_SENSE
static DEVICE_ATTR(es_enabled, 0664, sec_ts_es_enabled_show, sec_ts_es_enabled_store);
static DEVICE_ATTR(es_touch_count, 0444, sec_ts_es_touch_count_show, NULL);
#endif
...
int sec_ts_es_read_delta(struct sec_ts_data *ts)
{
	...
	readbytes = (ts->rx_count / 2) * ts->tx_count * 2;
	...
	/* read data */
	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_PROXIMITY_MUTU_DATA, pRead, readbytes);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: read rawdata failed!\n", __func__);
		goto ErrorRelease;
	}
	...
	for (i = 0; i < readbytes; i += 2) {
		ear_data[i] = pRead[i+1];
		ear_data[i+1] = pRead[i];
	}
...
int sec_ts_read_hover_data(struct sec_ts_data *ts)
{
	unsigned char *pRead = NULL;
	int ret = 0;
	int ii = 0;
...
//	input_dbg(true, &ts->client->dev, "%s\n", __func__);
	...
	/* read data */
	ret = ts->sec_ts_i2c_read(ts, SEC_TS_READ_PROXIMITY_HOVER_SELF_DATA, pRead, data_length);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: read rawdata failed!\n", __func__);
		goto err_read_data;
	}
	...
	for (ii = 0; ii < data_length; ii += 2) {
		hover_data[ii] = pRead[ii+1];
		hover_data[ii + 1] = pRead[ii];
	}
...
static ssize_t sec_ts_ear_data_show(struct file *data_file,
		...
		char *buf, loff_t pos, size_t count)
{
	...
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
	struct sec_ts_data *ts = container_of(sec, struct sec_ts_data, sec);
	int total_length = ((ts->rx_count/2) * ts->tx_count) * 2;
	int retry;

	if (count != total_length) {
		input_info(true, &ts->client->dev, "%s : bytes are not matched\n", __func__);
		return 0;
	}

	/* wait for reading ear_data */
	for (retry = 0; retry < SEC_TS_WAIT_RETRY_CNT; retry++) {
		if (!ts->read_ear_data)
			break;
	...
	input_info(true, &ts->client->dev, "%s : cnt - %d\n", __func__, retry);

	memcpy(buf, ear_data, total_length);

	return total_length;
}

static ssize_t sec_ts_hover_data_show(struct file *data_file,
		struct kobject *kobj, struct bin_attribute *attributes,
		char *buf, loff_t pos, size_t count)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct sec_cmd_data *sec = dev_get_drvdata(dev);
	struct sec_ts_data *ts = container_of(sec, struct sec_ts_data, sec);
	int total_length = (ts->rx_count + ts->tx_count) * 2;
	...
	sec_ts_read_hover_data(ts);

	memcpy(buf, hover_data, total_length);

	return total_length;
}

static inline struct device *ts_attr_kobj_to_dev(struct kobject *kobj)
{
	return container_of(kobj->parent, struct device, kobj);
}

static inline void *ts_attr_kobj_to_drvdata(struct kobject *kobj)
{
	return dev_get_drvdata(ts_attr_kobj_to_dev(kobj));
}

static struct bin_attribute ear_data_attr = {
	.attr = {
		.name = "ear_data",
		.mode = 0444,
	},
	.size = EAR_DATA_READ_LENGTH,
	.read = sec_ts_ear_data_show,
};

static struct bin_attribute hover_data_attr = {
	.attr = {
		.name = "hover_data",
		.mode = 0444,
	},
	.size = HOVER_DATA_READ_LENGTH,
	.read = sec_ts_hover_data_show,
};
#endif
...
int sec_ts_fn_init(struct sec_ts_data *ts)
{
...
#ifdef QEEXO_EAR_SENSE
	retval = sysfs_create_bin_file(&ts->sec.fac_dev->kobj,
			&ear_data_attr);
	if (retval < 0) {
		input_err(true, &ts->client->dev,
			"%s: Failed to create sysfs ear data bin file\n",
			__func__);
		goto exit;
	}
	...
	if (retval < 0) {
		input_err(true, &ts->client->dev,
			"%s: Failed to create sysfs hover data bin file\n",
			__func__);
		goto exit;
	}
#endif
...
#ifdef QEEXO_EAR_SENSE
	sysfs_remove_bin_file(&ts->sec.fac_dev->kobj, 
			&ear_data_attr);
	sysfs_remove_bin_file(&ts->sec.fac_dev->kobj, 
			&hover_data_attr);
#endif
