...
Copyright (C) 2012 - 2019, Samsung Electronics. All rights reserved.
...
static void TP_gamma_voltage_calc(struct SMART_DIM *pSmart)
{
	...
	for (j = 0; j < RGB_MAX; j++) {
		...
		fraction[V0][0] = v0_coefficient[MTP_OFFSET];
...
static void TP_gamma_code_calc(struct SMART_DIM *pSmart, int *M_GRAY, int *str)
{
	...
	str[0] = (v255_9bit[R] << 2) | (v255_9bit[G] << 1) | (v255_9bit[B]);
...
static void mtp_offset_substraction(struct SMART_DIM *pSmart, int *str)
{
	...
	/* V255 : str[0] ~ str[3] */
	v255_9bit[R] = !!(str[0] & BIT(2));
	v255_9bit[G] = !!(str[0] & BIT(1));
	v255_9bit[B] = !!(str[0] & BIT(0));

	level_V255[R] = str[1] | (v255_9bit[R] << 8);
	level_V255[G] = str[2] | (v255_9bit[G] << 8);
	level_V255[B] = str[3] | (v255_9bit[B] << 8);
...
static void Color_shift_compensation(struct SMART_DIM *pSmart, int *gamma_setting, int table_index)
{
	...
	v255_9bit[R] = !!(gamma_setting[0] & BIT(2));
	v255_9bit[G] = !!(gamma_setting[0] & BIT(1));
	v255_9bit[B] = !!(gamma_setting[0] & BIT(0));

	level_V255[R] = gamma_setting[1] | (v255_9bit[R] << 8);
	level_V255[G] = gamma_setting[2] | (v255_9bit[G] << 8);
	level_V255[B] = gamma_setting[3] | (v255_9bit[B] << 8);

	level_V255[R] += get_rgb_offset(table_index, 0, pSmart->panel_revision);
	level_V255[G] += get_rgb_offset(table_index, 1, pSmart->panel_revision);
	level_V255[B] += get_rgb_offset(table_index, 2, pSmart->panel_revision);

	v255_9bit[R] = !!(level_V255[R] & BIT(8));
	v255_9bit[G] = !!(level_V255[G] & BIT(8));
	v255_9bit[B] = !!(level_V255[B] & BIT(8));

	gamma_setting[0] = v255_9bit[R] << 2 | v255_9bit[G] << 1 | v255_9bit[B];
	...
	for (i = 3; i < RGB_COMPENSATION; i++) {
		...
		gamma_setting[i+1] += get_rgb_offset(table_index, i, pSmart->panel_revision);
	}
}
...
static void set_max_lux_table(int ldi_revision)
{
	...
	pr_err("%s ldi_revision: 0x%x\n", __func__, ldi_revision);

	v255_9bit[R] = (center_gamma[V255][R] & BIT(8)) >> 6;
	v255_9bit[G] = (center_gamma[V255][G] & BIT(8)) >> 7;
	v255_9bit[B] = (center_gamma[V255][B] & BIT(8)) >> 8;

	cnt = 0;
	max_lux_table[cnt++] = v255_9bit[R] | v255_9bit[G] | v255_9bit[B];

	/* V255 ~ V1 */
	for (v = V255; v >= V1; v--) {
		max_lux_table[cnt++] = center_gamma[v][R] & 0xFF;
		max_lux_table[cnt++] = center_gamma[v][G] & 0xFF;
		max_lux_table[cnt++] = center_gamma[v][B] & 0xFF;
	}

	/* V0, VT */
	max_lux_table[cnt++] =
		(center_gamma[V0][R] & 0xF) << 4 | (center_gamma[V0][G] & 0xF);
	max_lux_table[cnt++] =
		(center_gamma[V0][B] & 0xF) << 4 | (center_gamma[VT][R] & 0xF);
	max_lux_table[cnt++] =
		(center_gamma[VT][G] & 0xF) << 4 | (center_gamma[VT][B] & 0xF);
}
...
static void mtp_sorting(struct SMART_DIM *psmart)
{
	...
	/* V255 */
	cnt = 0;
	v255_9bit[R] = !!(pfrom[cnt] & BIT(2));
	v255_9bit[G] = !!(pfrom[cnt] & BIT(1));
	v255_9bit[B] = !!(pfrom[cnt] & BIT(0));
	cnt++;

	for (i = 0; i < RGB_MAX; i++) {
		psmart->MTP[V255][i] = pfrom[cnt++];
		if (v255_9bit[i])
			psmart->MTP[V255][i] *= -1;
	}
	...
	/* V0 */
	psmart->MTP[V0][R] = (pfrom[31] >> 4) & 0xF;
	psmart->MTP[V0][G] = pfrom[31] & 0xF;
	psmart->MTP[V0][B] = (pfrom[32] >> 4) & 0xF;

	/* VT */
	psmart->MTP[VT][R] = pfrom[32] & 0xF;
	psmart->MTP[VT][G] = (pfrom[33] >> 4) & 0xF;
	psmart->MTP[VT][B] = pfrom[33] & 0xF;
}

#define HBM_CANDELA 700
...
static void hbm_interpolation_init(struct SMART_DIM *pSmart)
{
	...
	v255_9bit[R] = !!(hbm_payload[0] & BIT(2));
	v255_9bit[G] = !!(hbm_payload[0] & BIT(1));
	v255_9bit[B] = !!(hbm_payload[0]);

	hbm_gamma[R] = (hbm_payload[R + 1] & 0xFF) | v255_9bit[R] << 8;
	hbm_gamma[G] = (hbm_payload[G + 1] & 0xFF) | v255_9bit[G] << 8;
	hbm_gamma[B] = (hbm_payload[B + 1] & 0xFF) | v255_9bit[B] << 8;
	...
	v255_9bit[R] = !!(max_lux_table[0] & BIT(2));
	v255_9bit[G] = !!(max_lux_table[0] & BIT(1));
	v255_9bit[B] = !!(max_lux_table[0]);

	max_gamma[R] = (max_lux_table[R + 1] & 0xFF) | (v255_9bit[R] << 8);
	max_gamma[G] = (max_lux_table[G + 1] & 0xFF) | (v255_9bit[G] << 8);
	max_gamma[B] = (max_lux_table[B + 1] & 0xFF) | (v255_9bit[B] << 8);
	...
	for (loop = 0; loop < hbm_steps; loop++) {
		...
		pSmart->hbm_interpolation_table[loop].gamma_setting[0] =
			(v255_9bit[R] << 2) | (v255_9bit[G] << 1) | (v255_9bit[B]);
...
static int smart_dimming_init(struct SMART_DIM *psmart)
{
	 ...
	 * 1. V255, VT, V0 : VREG1 - VREG1 * (OutputGamma + numerator_TP)/(denominator_TP)
	 * 2. V203 ~ V11 : VT - (VT - V_nextTP) * (OutputGamma + numerator_TP)/(denominator_TP)
	 * 3. V7 ~ V1 : VREG1 - (VREG1 - V_nextTP) * (OutputGamma + numerator_TP)/(denominator_TP)
	 */
