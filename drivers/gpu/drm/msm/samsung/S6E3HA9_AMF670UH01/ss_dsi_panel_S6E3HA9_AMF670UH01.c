...
*/
#include "ss_dsi_panel_S6E3HA9_AMF670UH01.h"
#include "ss_dsi_mdnie_S6E3HA9_AMF670UH01.h"
...
#define IRC_MODERATO_MODE_VAL	0xE5
#define IRC_FLAT_GAMMA_MODE_VAL	0xA5
...
static struct dsi_panel_cmd_set *ss_hbm_etc(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*/
	...
	/* read OTP B5h 58th -> write B5h 57h(Gram) */
	hbm_etc_cmds->cmds[1].msg.tx_buf[57] = (u8)vdd->br.elvss_value2;

	/* VINT */
	br_interpolation_generate_event(vdd, GEN_HBM_INTERPOLATION_VINT, &hbm_etc_cmds->cmds[2].msg.tx_buf[2]);
	...
	if (vdd->gradual_acl_val) {
		...
		hbm_etc_cmds->cmds[4].msg.tx_buf[1] = 0x02;	/* ACL ON */
		hbm_etc_cmds->cmds[3].msg.tx_buf[2] = 0x55;
	} else {
		hbm_etc_cmds->cmds[3].msg.tx_buf[6] = 0x0;	/* 0% */
	...
	*level_key = LEVEL1_KEY;
	...
	LCD_INFO("elvss 3th: 0x%x 0x%x, vint 0x%x, acl (%d %x, per=%x, frame avg=%x)\n",
...
static struct dsi_panel_cmd_set *ss_elvss(struct samsung_display_driver_data *vdd, int *level_key)
{
	...
	*/
	...
	elvss_cmds->cmds[0].msg.tx_buf[57] = (u8)vdd->br.elvss_value1;
...
{
	u8 valid_checksum[4] = {0x44, 0xa4, 0x44, 0xa4};
	int res;
...
static void samsung_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3HA9_AMF670UH01;

	/* GAMMA FLASH */
	vdd->panel_func.gen_hbm_interpolation_gamma = gen_hbm_interpolation_gamma_S6E3HA9_AMF670UH01;
	vdd->panel_func.gen_hbm_interpolation_irc = gen_hbm_interpolation_irc_S6E3HA9_AMF670UH01;
	vdd->panel_func.gen_normal_interpolation_irc = gen_normal_interpolation_irc_S6E3HA9_AMF670UH01;
	vdd->panel_func.samsung_flash_gamma_support = flash_gamma_support_S6E3HA9_AMF670UH01;
	vdd->panel_func.samsung_interpolation_init = init_interpolation_S6E3HA9_AMF670UH01;
	...
	vdd->panel_func.samsung_smart_get_conf_hmt = smart_get_conf_S6E3HA9_AMF670UH01_hmt;
...
static int __init samsung_panel_initialize(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3HA9_AMF670UH01_FHD";
