			 ...
			 size_t count, loff_t *ppos)
{
	...
	case FLAG_SELF_MASK:
		...
		break;
	case FLAG_SELF_ICON:
		make_self_display_img_cmds_6W3(vdd, TX_SELF_ICON_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_GRID:
		...
		break;
	case FLAG_SELF_ACLK:
		make_self_display_img_cmds_6W3(vdd, TX_SELF_ACLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		vdd->self_disp.operation[FLAG_SELF_DCLK].select = false;
		break;
	case FLAG_SELF_DCLK:
		make_self_display_img_cmds_6W3(vdd, TX_SELF_DCLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
...
int self_display_init_6W3(struct samsung_display_driver_data *vdd)
{
	...
	vdd->self_disp.aod_enter = NULL;
	vdd->self_disp.aod_exit = NULL;
	vdd->self_disp.self_mask_check = NULL;
	vdd->self_disp.self_move_set = NULL;
	vdd->self_disp.self_icon_set = NULL;
	vdd->self_disp.self_aclock_set = NULL;
	vdd->self_disp.self_dclock_set = NULL;
	vdd->self_disp.self_time_set = NULL;
	vdd->self_disp.self_partial_hlpm_scan_set = NULL;
	vdd->self_disp.self_blinking_on = NULL;
	vdd->self_disp.self_display_debug = NULL;
